...
if [ -z "$LEARNHOUSE_DOMAIN" ]; then
    export LEARNHOUSE_DOMAIN="iqexponencial.com"
fi
if [ -z "$LEARNHOUSE_FRONTEND_DOMAIN" ] && [ -n "$LEARNHOUSE_DOMAIN" ]; then
    export LEARNHOUSE_FRONTEND_DOMAIN="$LEARNHOUSE_DOMAIN"
fi
if [ -z "$LEARNHOUSE_ALLOWED_ORIGINS" ] && [ -n "$LEARNHOUSE_DOMAIN" ]; then
    export LEARNHOUSE_ALLOWED_ORIGINS="https://$LEARNHOUSE_DOMAIN"
fi
if [ -z "$LEARNHOUSE_COOKIE_DOMAIN" ] && [ -n "$LEARNHOUSE_DOMAIN" ]; then
    export LEARNHOUSE_COOKIE_DOMAIN=".$LEARNHOUSE_DOMAIN"
fi
if [ -z "$LEARNHOUSE_SSL" ]; then
    export LEARNHOUSE_SSL="true"
fi

if [ -z "$NEXT_PUBLIC_LEARNHOUSE_DOMAIN" ] && [ -n "$LEARNHOUSE_DOMAIN" ]; then
    export NEXT_PUBLIC_LEARNHOUSE_DOMAIN="$LEARNHOUSE_DOMAIN"
fi
if [ -z "$NEXT_PUBLIC_LEARNHOUSE_TOP_DOMAIN" ] && [ -n "$LEARNHOUSE_DOMAIN" ]; then
    export NEXT_PUBLIC_LEARNHOUSE_TOP_DOMAIN="$LEARNHOUSE_DOMAIN"
fi
if [ -z "$NEXT_PUBLIC_LEARNHOUSE_HTTPS" ]; then
    export NEXT_PUBLIC_LEARNHOUSE_HTTPS="true"
fi
if [ -z "$NEXT_PUBLIC_LEARNHOUSE_BACKEND_URL" ] && [ -n "$LEARNHOUSE_DOMAIN" ]; then
    export NEXT_PUBLIC_LEARNHOUSE_BACKEND_URL="https://$LEARNHOUSE_DOMAIN/"
fi
if [ -z "$NEXT_PUBLIC_LEARNHOUSE_API_URL" ] && [ -n "$LEARNHOUSE_DOMAIN" ]; then
    export NEXT_PUBLIC_LEARNHOUSE_API_URL="https://$LEARNHOUSE_DOMAIN/api/v1/"
fi
if [ -z "$NEXT_PUBLIC_COLLAB_URL" ] && [ -n "$LEARNHOUSE_DOMAIN" ]; then
    export NEXT_PUBLIC_COLLAB_URL="wss://$LEARNHOUSE_DOMAIN/collab"
fi
