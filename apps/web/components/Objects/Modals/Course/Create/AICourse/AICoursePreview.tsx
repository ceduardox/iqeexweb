...
function PlanChapterActivityRow({
  activity,
  chapterIndex,
  activityIndex,
  plan,
  ...
  React.useEffect(() => {
    setVal(activity.name)
  }, [activity.name])

  const save = () => {
    if (val.trim()) {
      const c = [...plan.chapters]
      const a = [...c[chapterIndex].activities]
      a[activityIndex] = { ...a[activityIndex], name: val.trim() }
      c[chapterIndex] = { ...c[chapterIndex], activities: a }
      onUpdatePlan({ ...plan, chapters: c })
    }
    setEditing(false)
  }
  ...
  const remove = () => {
    ...
    c[chapterIndex] = {
      ...c[chapterIndex],
      activities: c[chapterIndex].activities.filter((_, i) => i !== activityIndex),
    }
    onUpdatePlan({ ...plan, chapters: c })
  }
      ...
      <div className="flex-1 min-w-0">
        {editing ? (
          <input
            ...
            className="w-full bg-white/10 rounded px-2 py-1 text-white text-sm focus:outline-none focus:ring-1 focus:ring-purple-500"
            autoFocus
          />
        ) : (
          <span onClick={() => setEditing(true)} className="text-sm text-white/80 hover:text-purple-300 cursor-pointer">
            {activity.name}
          </span>
        )}
